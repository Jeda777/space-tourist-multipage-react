...
type Props = {
  i: { name: string; index: number; role: string; bio: string }
}

const Member: React.FC<Props> = ({ i }) => {
  ...
  return (
    <div
      {...handlers}
      className='flex flex-col-reverse md:flex-col xl:flex-row items-center gap-10 text-center xl:text-left w-full 
        xl:justify-around'
    >
        ...
        <div className='flex gap-2'>
          <button
            className={`w-[10px] h-[10px] rounded-full bg-white ${
              i.role === 'Commander' ? '' : 'opacity-50'
            } hover:opacity-75 transition`}
            onClick={() => navigate('../commander')}
          ></button>
          <button
            className={`w-[10px] h-[10px] rounded-full bg-white ${
              i.role === 'Mission Specialist' ? '' : 'opacity-50'
            } hover:opacity-75 transition`}
            onClick={() => navigate('../mission-specialist')}
          ></button>
          <button
            className={`w-[10px] h-[10px] rounded-full bg-white ${
              i.role === 'Pilot' ? '' : 'opacity-50'
            } hover:opacity-75 transition`}
            onClick={() => navigate('../pilot')}
          ></button>
          <button
            className={`w-[10px] h-[10px] rounded-full bg-white ${
              i.role === 'Flight Engineer' ? '' : 'opacity-50'
            } hover:opacity-75 transition`}
      ...
      <div id='crew-img-container'>
        <img
          className='max-w-[180px] sm:max-w-[220px] md:max-w-[350px] lg:max-w-[400px] xl:max-w-[600px]'
          src={members[i.index]}
          alt={i.name}
        />
      </div>
