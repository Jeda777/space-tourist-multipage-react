...
type Props = {
  i: { name: string; index: number; description: string }
}

const ExampleTech: React.FC<Props> = ({ i }) => {
  ...
  return (
    <div {...handlers} className='grid grid-cols-1 justify-items-center gap-8 xl:grid-cols-[1fr_6fr_5fr] xl:items-center'>
      <div className='text-white flex gap-4 justify-center order-2 xl:order-1 xl:flex-col'>
        <button
          className={`w-10 h-10 md:w-12 md:h-12 md:text-xl lg:w-14 lg:h-14 lg:text-2xl xl:w-20 xl:h-20 xl:text-3xl
                    rounded-full border-[1px] border-white border-opacity-25 hover:border-opacity-100 transition-all
                    ${i.name === 'Launch vehicle' ? 'tab-active' : ''}`}
          onClick={() => navigate('../launch-vehicle')}
        >
          1
        </button>
        <button
          className={`w-10 h-10 md:w-12 md:h-12 md:text-xl lg:w-14 lg:h-14 lg:text-2xl xl:w-20 xl:h-20 xl:text-3xl
                    rounded-full border-[1px] border-white border-opacity-25 hover:border-opacity-100 transition-all
                    ${i.name === 'Spaceport' ? 'tab-active' : ''}`}
          onClick={() => navigate('../spaceport')}
        >
          2
        </button>
        <button
          className={`w-10 h-10 md:w-12 md:h-12 md:text-xl lg:w-14 lg:h-14 lg:text-2xl xl:w-20 xl:h-20 xl:text-3xl
                    rounded-full border-[1px] border-white border-opacity-25 hover:border-opacity-100 transition-all
                    ${i.name === 'Space capsule' ? 'tab-active' : ''}`}
          onClick={() => navigate('../space-capsule')}
        >
          3
        </button>
      </div>
      ...
      <img src={technologiesLandscape[i.index]} alt={i.name} className='xl:hidden order-1' />

      <img src={technologiesPortrait[i.index]} alt={i.name} className='hidden xl:inline-block order-3' />
    </div>
  )
}
